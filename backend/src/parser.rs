...
pub fn parse_number<'a>(input : &'a str) -> (Option<usize> , &'a str) {
    ...
    if !c.is_numeric() { return (None, input); }
...
pub fn parse_whitespace<'a>(input : &'a str) -> (Option<&'a str>, &'a str) {
    Regex::new(r"^\s+").unwrap().find(input).map(|m| (Some(m.as_str()), &input[m.end()..])).unwrap_or((None, input))
}
...
pub fn parse_piece_fen<'a>(input : &'a str) -> (Option<&'a str>, &'a str) {
    if input.len() < 2 { return (None, input); }
    let test = &input[0..2];
    match test {
        "NN" => (Some("NN"), &input[2..]),
        "WW" => (Some("WW"), &input[2..]),
